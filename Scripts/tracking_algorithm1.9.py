...
def calc_clusters(data_input, algorithm="kmeans", n_clusters=10, plot=True):
    valid_algorithms = ["kmeans", "dbscan", "mikes", "none", "hdbscan", "gmm", "htdbscan"]
    algorithm_t = "dbscan"
    ...
    # Calculate the predictions using Mike distance tracking method
        ...
        p1, p2, p3 = calc_hough_transform(data, plot=True)
        print("Clustering H-space...")
        sys.exit()
        # Remove the duplicates
        p1 = np.column_stack((p1, p2))  # This operation is slow, not as slow as clustering lol
        print("Done stacking")
        model = DBSCAN(eps=13, min_samples=5, n_jobs=2)
        ...
        for i in range(len(pred_y)):
            clusters[pred_y[i]][1].append([p1[i, 0], p1[i, 1], p1[i, 2], p1[i, 3]])
...
def calc_hough_transform(data_input, origin=(0, 0), plot=True):
    """
    ...
    tH1 = __hough_transform2(X[:, [0, 2]], origin=origin, sequence=False)
    tH2 = __hough_transform2(X[:, [1, 2]], origin=origin, sequence=False)
    tH3 = __hough_transform2(X[:, [0, 1]], origin=origin, sequence=False)
    if plot:
...
# calc_clusters(data, algorithm="mikes"))


# Cluster and plot the clusters for their 2D projections
calc_clusters(data, algorithm=input("'kmeans', 'dbscan', 'mikes', 'none', 'hdbscan', 'gmm', 'htdbscan' ? "))
