...
def calc_clusters(data_State, algorithm="kmeans", n_clusters=10, plot=True, plot_type='2d', heatmap=False,
    ...
    if algorithm_t.lower() != 'mike-htdbscan' or algorithm_t.lower() != 'mike-hthdbscan':
...
def evaluate_success(algorithm='dbscan', tp='49', verbose=False, plot=True, max_u_value=-1.0, mean_line=True)->list:
    """
...
# run_main(algorithm=input("'kmeans', 'dbscan', 'mike', 'none', 'hdbscan', 'gmm', 'richard-dbscan', 'mike-htdbscan' ? "))

# produce_histogram(draw_type='bar', bin_count=100, cover='00', cutoff=-1, plot=True)

# evaluate_success('kmeans', tp='49', verbose=True, max_u_value=3, mean_line=True)

# export_to_excel(data, "MDM3_MOJO2")  # Export data to a spreadsheet
# run_main(algorithm='mike-hthdbscan', cover='04', plot=True)
# run_main(algorithm='dbscan', cover='04', plot=True)
