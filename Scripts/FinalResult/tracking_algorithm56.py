...
class State2:
    def __init__(self, data_input, cover='00', tp='49'):
        ...
        self.X3 = np.array([[sperm["center"][0], sperm["center"][1], i] for frame in data_input["centroids"]
                            for i, sperm in enumerate(frame)])
        self.cover = cover
...
class State3:
    def __init__(self, X, cover='00', tp='49'):
        ...
        self.X3 = [[[self.x[j], self.y[j], self.z[j]] for j in range(int(number))] for number in _indices_diffs]
...
def _linear_rescale(_x: list, _min: float, _max: float, rnd: int=2):
    ...
    #     print('Division error for: {}'.format(_x))
    #     print('Max: {}, Min: {}'.format(_o_max, min(_x)))
    #     sys.exit(1)
    b = _max - a * _o_max
...
def _generate_colors(n, rnd: int = 2):
    ...
    colors = _linear_rescale(list(range(n)), 0, 1, rnd=rnd)
...
def _write_clusters_space_to_json(predictions, cover='00', tp='49', algorithm='none', verbose=False):
    ...
    if len(predictions) > len(State2(_old_data)):
...
def _plot_ellipse(x, y, xr, yr, ax, color='black', linewidth=1.0, rotation=0, precision=100):
    t = np.linspace(0, 2 * math.pi, precision)

    # x, y removed to keep the same center location
    Ell = np.array([xr * np.cos(t), yr * np.sin(t)])

    # 2-D rotation matrix
    R_rot = np.array([[math.cos(rotation), -math.sin(rotation)], [math.sin(rotation), math.cos(rotation)]])
    ...
    if algorithm == 'ground-truth':
        ...
        for i, _tp in enumerate(tps):
            for cover in acceptable_coversG:
                # Dummy State object
                n_f = num_frames(import_data(acceptable_tpG, cover=cover, tp=_tp))

                clusters1, _ = csv_to_clusters(r'For Mike IDL\tp{}\MHT{}_{}.csv'.format(_tp, cover[0], cover[1]))
                u_all[i].append(calc_u_value(clusters1, n_f, verbose=True))

    elif algorithm == 'JPDAF':
        for i, _tp in enumerate(tps):
            for cover in acceptable_coversG:
                # Dummy State object
                n_f = num_frames(import_data(acceptable_tpG, cover=cover, tp=_tp))

                clusters1, _ = csv_to_clusters(r'JPDA Res\tp{}\JPDA{}_{}.csv'.format(_tp, cover[0], cover[1]))
                u_all[i].append(calc_u_value(clusters1, n_f, verbose=True))
...
def clusters_to_csv(clusters, path):

    X = []

    for cluster in clusters:
        for sperm in cluster[1]:
            sperm_temp = sperm.copy()
            sperm_temp.append(cluster[0])
            X.append(sperm_temp)
...
def csv_to_clusters(path):
    ...
    if not temp_path.is_file():
        ...
        # Convert the ID row into integers first
        try:
            data[:, 3] = data[:, 3].astype(int)
        except TypeError:
            print('Type error, file header given {}'.format(data[0, :]))
            sys.exit(1)

        smallest_id = min(data[:, 3])
        if smallest_id >= 1:
            data[:, 3] = data[:, 3] - smallest_id
...
def plot_annotations(ax1, ax2, ax3, algorithm, tp, cover):
    ...
    if algorithm == 'ground-truth' and tp == '49' and cover == '04':
        ...
        _annotate('Discontuinities', ax3, xy=(220, 75), xytext=(276, -11), face_color='red',
                  shrink=0.05, text_color='red')

        # Kmeans 49, 04, annotate linearity
        _plot_ellipse(158, 150, 26, 160, ax2, color='red', linewidth=2.0, rotation=math.pi / 32)
    ...
    if algorithm == 'IDL' and tp == '49' and cover == '04':
        _plot_ellipse(296, 67, 80, 80, ax3, color='red', linewidth=2.0, rotation=0)
        _annotate('Perfect resolution', ax3, xy=(330, 47), xytext=(400, -20), face_color='red', shrink=1.6,
                  text_color='red',
                  text_size=14)

        _plot_ellipse(135, 554, 56, 46, ax1, color='red', linewidth=2.0, rotation=0)
        _annotate('Should be\nseparate', ax1, xy=(161, 542), xytext=(220, 417), face_color='red', shrink=1.6,
                  text_color='red',
                  text_size=14)

    if algorithm == 'JPDAF' and tp == '49' and cover == '04':
        _plot_ellipse(606, 319, 50, 90, ax1, color='red', linewidth=2.0, rotation=math.pi / 16)
        _annotate('Smoothly\ninterpolated', ax1, xy=(586, 331), xytext=(203, 406), face_color='red', shrink=1.6,
                  text_color='red',
                  text_size=14)
    ...
    if algorithm == 'MHT' and tp == '49' and cover == '04':
        _plot_ellipse(606, 319, 50, 90, ax1, color='red', linewidth=2.0, rotation=math.pi / 16)
        _annotate('Smoothly\ninterpolated', ax1, xy=(586, 331), xytext=(203, 406), face_color='red', shrink=1.6,
                  text_color='red',
                  text_size=14)
        ...
        _plot_ellipse(115, 160, 50, 60, ax2, color='red', linewidth=2.0, rotation= 0)
        _annotate('Correctly\nsplit', ax2, xy=(96, 154), xytext=(-10, 70), face_color='red', shrink=1.5,
                  text_color='red',
                  text_size=14)

    if algorithm == 'MHT / orientations' and tp == '49' and cover == '04':
        pass


def plot_clusters(clusters, frame_num, algorithm='None', tp='49', cover='00', plot_type='2d', heatmap=False,
                  legend=False, annotations=True, scale=1.0):
    n_clusters = len(clusters)
    ...
    if plot_type == '2d':
        ...
        title = '{} clustering applied to the sperm centroids for tp {} cover {}'.format(algorithm_t.upper(),
                                                                                         tp,
                                                                                         cover)
        if n_clusters < 40:
            # Small number
        ...
        if algorithm_t.lower() == 'ground-truth':
            fig.suptitle(
                'Ground truth for sperm centroids for tp {} cover {}'.format(tp, cover),
                fontsize=20)

        for i, cluster in enumerate(clusters_asarr):
            color_map = colors[i]
            ax1.scatter(cluster[:, 0] * (1 / scale), cluster[:, 1] * (1 / scale), label=i, color=color_map, s=5)  # X vs Y     (C)
            ax2.scatter(cluster[:, 1] * (1 / scale), cluster[:, 2], label=i, color=color_map, s=5)  # Y vs Frame (B)
            ax3.scatter(cluster[:, 0] * (1 / scale), cluster[:, 2], label=i, color=color_map, s=5)  # X vs Frame (A)
        ...
        fig.tight_layout(rect=[0, 0.03, 1, 0.95])  # Rescale everything so subplots always fit
        
        plt.show()
    elif plot_type == '2d-xy':
        fig, (ax1) = plt.subplots(1, 1)

        title = '{} clustering for sperm centroids for tp {} cover {}'.format(algorithm_t.upper(),
                                                                                              tp,
                                                                              cover)
        if algorithm_t.lower() == 'ground-truth':
            title = 'Ground truth for sperm centroids for tp {} cover {}'.format(algorithm_t.upper(),
                                                                                             tp,
                                                                                             cover)

        title_length = len(title)

        print('Title length:  {}'.format(title_length))

        # Pad the title based on the length
        figure_title_pad = 7.5
        if 90 >= title_length > 85:
            figure_title_pad = 6.5
        elif 85 >= title_length > 72:
            figure_title_pad = 5
        elif 72 >= title_length > 60:
            figure_title_pad = 3

        print(figure_title_pad)

        if n_clusters < 40:
            # Small number
            fig.set_size_inches(6 + figure_title_pad, 5)  # Set the sizing
        elif n_clusters < 80:
            # Medium number
            fig.set_size_inches(7 + figure_title_pad, 5)  # Set the sizing
        elif n_clusters < 150:
            # Large number
            fig.set_size_inches(8 + figure_title_pad, 6)  # Set the sizing
        else:
            # Very Large number
            fig.set_size_inches(9 + figure_title_pad, 7)  # Set the sizing
        ...
        for i, cluster in enumerate(clusters_asarr):
            color_map = colors[i]
            ax.scatter(cluster[:, 0] * (1 / scale), cluster[:, 1] * (1 / scale), cluster[:, 2], color=color_map, s=1)
...
def calc_clusters(data_State, algorithm="kmeans", n_clusters=10, plot=True, plot_type='2d', heatmap=False,
    ...
    if data_State.frame_num < min_points_for_clustering:
    ...
    elif algorithm_t.lower() == 'richard-bic-dbscan':
        # Calculate the predictions using Mike distance tracking method
        # Precalculate the euclidean distances of each sperm from each other sperm for every frame with frame diff
        n_beginning = len(data_State)
        ...
        for speram_number in range(1, len(new_lists_cluster)):
            if 300 >= len(new_lists_cluster[speram_number][1]) >= 10:
                count2 += 1
        ...
        algorithm_t = 'DBSCAN / BIC-polynomial / DBSCAN'
        ...
        algorithm_t = 'BIC-polynomial 2-HDBSCAN'
        ...
        print('Currently not working with: {}'.format(algorithm_t))
        sys.exit(1)
    ...
    if algorithm_t.lower() != 'closest frame / hough transform' \
        ...
        # Append all of the clustering predictions to the data structure
        # Quickly convert the predictions to integers
        pred_y = [int(prediction) for prediction in pred_y]
...
if __name__ == '__main__':
    ...
    # csv_to_json('IDL_tracks0_4.csv', '49', '04', algorithm="IDL")
    #
    # clusters1, _ = csv_to_clusters('JPDA.csv')
    # data04 = import_data(acceptable_tpG, cover='04', tp='49')
    # plot_clusters(clusters1, State2(data04).frame_num, algorithm="MHT / orientations", cover='04', tp='49', scale=2)
