...
def calc_clusters(data_State, algorithm="kmeans", n_clusters=10, plot=True, plot_type='2d', heatmap=False,
                  min_points_for_clustering=20, return_clusters=False):
    valid_algorithms = ["kmeans", "dbscan", "mike", "none", "hdbscan", "gmm", "htdbscan", "richard-dbscan", 'mike-htdbscan']
    ...
    if data_State.frame_num < min_points_for_clustering:
        ...
        # How many clusters?
        n = len(set(labels)) - (1 if -1 in labels else 0) + 1
        n_noise = list(labels).count(-1)
        print('# of clusters ', n)
        print('# of Noise clusters ', n_noise)
    ...
    if algorithm_t.lower() != 'mike-htdbscan':
        # Generate the ID and a random colours for each ID
        # List comprehensions are over 100% more efficient
        clusters = [[i, []] for i in range(n)]
    ...
    colors = [[random.uniform(0.05, 0.7), random.uniform(0.05, 0.7), random.uniform(0.05, 0.7)] for _ in range(n)]
    ...
    return data_State.X, pred_y


def __hough_transform3(data_input, origin=(0, 0)):
    # Only use this for sequenced data
...
def __hough_transform2(data_input, origin=(0, 0), sequence=False):
    """
    ...
    if sequence:
        ...
        X2 = X  # Faster RAM accessing times
...
def evaluate_success(algorithm='dbscan', tp='49', verbose=False, plot=True, max_u_value=-1.0)->list:
    """
    ...
    """
    u_all = []
    p = len(acceptable_coversG)  # Number of covers
    if verbose:
        print('Calculating U for {} covers'.format(p))
        for i, cover in enumerate(acceptable_coversG):
            print('Done Cover {} of {}'.format(i+1, p))
            data = import_data(acceptable_tpG, cover=cover, tp=tp, verbose=verbose)
            data = State2(data, cover=cover, tp=tp)
            u_all.append(calc_clusters(data, plot=False, algorithm=algorithm, plot_type='bar_graph'))
    else:
        for i, cover in enumerate(acceptable_coversG):
            data = import_data(acceptable_tpG, cover=cover, tp=tp, verbose=verbose)
            data = State2(data, cover=cover, tp=tp)
    ...
    if plot:
        ...
        # Plot the data
        fig, ax = plt.subplots()
        fig.suptitle('Evaluating {} for tp {}'.format(algorithm, tp),
                     fontsize=18)
        ...
        if verbose:
            for x_val, y_val, color_val in zip(acceptable_coversG, u_all, colors):
                print('PLOT: {} , {}, {}'.format(x_val, y_val, color_val))
                ax.bar(x_val, y_val, label=tp, color=color_val)
        else:
            for x_val, y_val, color_val in zip(acceptable_coversG, u_all, colors):
                ax.bar(x_val, y_val, label=tp, color=color_val)
...
# Obviously change your path to the directory with the data if this code returns errors.
# len(data["centroids"][i]) is the number of sperms in a frame. This is subject to change each frame
...
# run_main(algorithm=input("'kmeans', 'dbscan', 'mike', 'none', 'hdbscan', 'gmm', 'htdbscan', 'richard-dbscan', 'mike-htdbscan' ? "))

# produce_histogram(draw_type='bar', bin_count=100, cover='00', cutoff=-1, plot=True)

# evaluate_success('kmeans', tp='57', verbose=True, max_u_value=3)

# run_main(algorithm='mike-htdbscan', cover='00', plot=True)
# run_main(algorithm='dbscan', cover='00', plot=True)
