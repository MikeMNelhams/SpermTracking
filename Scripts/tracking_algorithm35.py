...
def calc_clusters(data_State, algorithm="kmeans", n_clusters=10, plot=True, plot_type='2d', heatmap=False):
    ...
    if plot:
        if plot_type == '2d':
            ...
            # colors = [plt.cm.viridis(totals[i] / max(totals)) for i in range(n)] Old
    ...
    if plot_type == 'bar_graph':
        ...
        # U = n_f - 1/n * SUM( abs( n_f - x) / n_f ) where x is the number of points in each cluster
        totals = [cluster.shape[0] for cluster in clusters_asarr]
        u = [abs(data_State.frame_num - n) for n in totals]
        u = np.array(u)
        u = data_State.frame_num / (np.mean(u))
        print(totals)
        print(u)
        return u
...
def evaluate_success(algorithm='dbscan', tp='49', verbose=False, plot=True):
    """
    :param algorithm: str
    :param tp: str (number)
    :param verbose: bool
    ...
    if plot:
        # Technically, this is deprecated, but try to stop me >:`)
        # colors = [plt.cm.viridis(u_all[i] / max(u_all)) for i in range(p)]
        viridis = cm.get_cmap('viridis', p)

        colors = [viridis(u_all[i] / max(u_all)) for i in range(p)]
        fig, ax = plt.subplots()
...
# run_main(algorithm='dbscan', cover='13', verbose=True, plot_type='bar_graph')
# run_main(algorithm='dbscan', cover='13', plot=True, verbose=True, plot_type='3d')

# produce_histogram(draw_type='bar', bin_count=100, cover='00', cutoff=-1, outlierline=True)
# run_main(algorithm='kmeans', cover='13', plot=False)
# r = produce_histogram(draw_type='bar', bin_count=100, cover='00', cutoff=-2, plot=False)
# print(r)  # RMS

# evaluate_success('kmeans', tp='57', verbose=True)

# run_main(algorithm='dbscan', cover='00', plot=True, plot_type='2d')
